import type { AirlineInfo, SearchParams, FlightResult, FlightLeg, BaggageInfo, FlightGeneratorConfig } from './flights.types';
import { getCityName, getCityCoords, getCityCountry, getPrimaryAirportCode } from './airport.utils';
...
function formatDate(dateStr: string | undefined, dayOffset: number = 0): string | undefined {
...
const AIRLINE_IATA: Record<string, string> = {
  ...
  'Delta': 'DL', 'United': 'UA', 'American': 'AA', 'Emirates': 'EK',
  'Qatar Airways': 'QR', 'Singapore Airlines': 'SQ', 'Cathay Pacific': 'CX',
  'Korean Air': 'KE', 'ANA': 'NH', 'JAL': 'JL', 'KLM': 'KL',
  'Swiss': 'LX', 'Aer Lingus': 'EI', 'Iberia': 'IB',
  'TAP Air Portugal': 'TP', 'SAS': 'SK', 'Turkish Airlines': 'TK',
  'Etihad Airways': 'EY', 'JetBlue': 'B6', 'Southwest Airlines': 'WN',
  'Alaska Airlines': 'AS', 'Hainan Airlines': 'HU',
};

function generateFlightNumber(rng: () => number, airlineName: string): string {
...
const WIDE_BODY = [
  'Boeing 777-300ER', 'Boeing 787-9 Dreamliner', 'Airbus A350-900',
  'Boeing 777-200LR', 'Airbus A330-300', 'Boeing 787-10 Dreamliner',
];
const FLAGSHIP_WIDE_BODY = [
  'Airbus A380-800', 'Boeing 777-300ER', 'Airbus A350-1000',
];
const NARROW_BODY = [
  'Airbus A321neo', 'Boeing 737 MAX 8', 'Airbus A320neo', 'Boeing 737-800',
];

function pickAircraft(rng: () => number, distanceKm: number, isPremiumRoute: boolean): string {
  ...
  if (distanceKm > 2000) {
    return rng() < 0.4
      ? WIDE_BODY[Math.floor(rng() * WIDE_BODY.length)]
      : NARROW_BODY[Math.floor(rng() * NARROW_BODY.length)];
  }
...
function pickFareClass(rng: () => number, cabin: string): string {
...
const CODESHARE_PARTNERS: Record<string, string[]> = {
  'British Airways': ['Iberia', 'American', 'Aer Lingus'],
  'Lufthansa': ['Swiss', 'SAS'],
  'Delta': ['Air France', 'KLM'],
  'United': ['ANA', 'Lufthansa'],
  'American': ['British Airways', 'JAL', 'Iberia'],
  ...
  'ANA': ['United'],
  'JAL': ['American', 'British Airways'],
  'Korean Air': ['Delta'],
  'Cathay Pacific': ['British Airways', 'American'],
};

function getCodesharePartner(rng: () => number, airlineName: string, probability: number): string | undefined {
...
type Region = 'americas' | 'europe' | 'middle_east' | 'east_asia' | 'south_asia' | 'oceania' | 'africa';

const COUNTRY_REGION: Record<string, Region> = {
  'United States': 'americas', 'Canada': 'americas', 'Mexico': 'americas',
  'Brazil': 'americas', 'Argentina': 'americas', 'Colombia': 'americas',
  'Peru': 'americas', 'Chile': 'americas', 'Costa Rica': 'americas',
  'Panama': 'americas', 'Dominican Republic': 'americas', 'Jamaica': 'americas',
  'Puerto Rico': 'americas', 'Bahamas': 'americas', 'Bermuda': 'americas',
  'Barbados': 'americas', 'Trinidad and Tobago': 'americas',
  'Cayman Islands': 'americas', 'Ecuador': 'americas', 'Guatemala': 'americas',
  ...
  'Croatia': 'europe', 'Iceland': 'europe', 'Bulgaria': 'europe',
  'Serbia': 'europe', 'Luxembourg': 'europe', 'Malta': 'europe', 'Cyprus': 'europe',
  'United Arab Emirates': 'middle_east', 'Qatar': 'middle_east',
  'Saudi Arabia': 'middle_east', 'Bahrain': 'middle_east', 'Oman': 'middle_east',
  'Kuwait': 'middle_east', 'Jordan': 'middle_east', 'Israel': 'middle_east',
  'Lebanon': 'middle_east', 'Turkey': 'middle_east', 'Türkiye': 'middle_east',
  'Japan': 'east_asia', 'South Korea': 'east_asia', 'China': 'east_asia',
  'Taiwan': 'east_asia', 'Hong Kong': 'east_asia', 'Singapore': 'east_asia',
  'Malaysia': 'east_asia', 'Thailand': 'east_asia', 'Vietnam': 'east_asia',
  'Philippines': 'east_asia', 'Indonesia': 'east_asia', 'Cambodia': 'east_asia',
  'Myanmar': 'east_asia',
  'India': 'south_asia', 'Pakistan': 'south_asia', 'Sri Lanka': 'south_asia',
  'Bangladesh': 'south_asia', 'Nepal': 'south_asia', 'Maldives': 'south_asia',
  'Australia': 'oceania', 'New Zealand': 'oceania', 'Fiji': 'oceania',
  'South Africa': 'africa', 'Kenya': 'africa', 'Nigeria': 'africa',
  'Egypt': 'africa', 'Morocco': 'africa', 'Ethiopia': 'africa',
  'Tanzania': 'africa', 'Ghana': 'africa', 'Tunisia': 'africa', 'Senegal': 'africa',
};

function getRegion(cityCode: string): Region | null {
  const country = getCityCountry(cityCode);
...
type RouteType =
  | 'transatlantic_east' | 'transatlantic_west'
  | 'us_to_middle_east' | 'middle_east_to_us'
  | 'us_to_east_asia' | 'east_asia_to_us'
  | 'us_to_south_asia' | 'south_asia_to_us'
  | 'europe_to_east_asia' | 'east_asia_to_europe'
  | 'intra_europe' | 'domestic_us'
  | 'short_haul' | 'long_haul';

function detectRouteType(fromCode: string, toCode: string, distanceKm: number): RouteType {
  const fromRegion = getRegion(fromCode);
  const toRegion = getRegion(toCode);
  const fromCountry = getCityCountry(fromCode);
  const toCountry = getCityCountry(toCode);
...
export function getAirlineLogo(name: string): string {
...
const ROUTE_AIRLINES: Record<RouteType, string[]> = {
  transatlantic_east: [
    'British Airways', 'Air France', 'Lufthansa', 'Delta', 'United', 'American',
    'KLM', 'Swiss', 'Aer Lingus', 'Iberia', 'TAP Air Portugal', 'SAS',
  ],
  transatlantic_west: [
    'British Airways', 'Air France', 'Lufthansa', 'Delta', 'United', 'American',
    'KLM', 'Swiss', 'Aer Lingus', 'Iberia', 'TAP Air Portugal', 'SAS',
  ],
  us_to_middle_east: ['Emirates', 'Qatar Airways', 'Etihad Airways', 'Turkish Airlines', 'Delta', 'United'],
  middle_east_to_us: ['Emirates', 'Qatar Airways', 'Etihad Airways', 'Turkish Airlines', 'Delta', 'United'],
  us_to_east_asia: ['ANA', 'JAL', 'Singapore Airlines', 'Cathay Pacific', 'Korean Air', 'Delta', 'United'],
  east_asia_to_us: ['ANA', 'JAL', 'Singapore Airlines', 'Cathay Pacific', 'Korean Air', 'Delta', 'United'],
  us_to_south_asia: ['Emirates', 'Qatar Airways', 'Etihad Airways', 'British Airways', 'Air France', 'Lufthansa'],
  south_asia_to_us: ['Emirates', 'Qatar Airways', 'Etihad Airways', 'British Airways', 'Air France', 'Lufthansa'],
  europe_to_east_asia: [
  ...
  intra_europe: ['Lufthansa', 'Air France', 'British Airways', 'KLM', 'Swiss', 'Iberia', 'SAS', 'Aer Lingus'],
  domestic_us: ['Delta', 'United', 'American', 'JetBlue', 'Southwest Airlines', 'Alaska Airlines'],
  short_haul: ['Delta', 'United', 'American', 'British Airways', 'Lufthansa', 'Air France', 'KLM'],
  long_haul: [
    'Emirates', 'Qatar Airways', 'Singapore Airlines', 'British Airways',
    'Lufthansa', 'Delta', 'United', 'Turkish Airlines',
  ],
};

// ── Route-specific layover cities ──

const ROUTE_LAYOVERS: Record<RouteType, string[]> = {
  transatlantic_east: ['Dublin', 'Reykjavik', 'London', 'Paris'],
  transatlantic_west: ['Dublin', 'Reykjavik', 'London', 'Paris'],
  us_to_middle_east: ['London', 'Paris', 'Frankfurt', 'Istanbul'],
  middle_east_to_us: ['London', 'Paris', 'Frankfurt', 'Istanbul'],
  us_to_east_asia: ['Seoul', 'Taipei', 'Tokyo', 'Honolulu'],
  east_asia_to_us: ['Seoul', 'Taipei', 'Tokyo', 'Honolulu'],
  us_to_south_asia: ['Dubai', 'Doha', 'Abu Dhabi', 'London'],
  south_asia_to_us: ['Dubai', 'Doha', 'Abu Dhabi', 'London'],
  europe_to_east_asia: ['Istanbul', 'Dubai', 'Doha', 'Helsinki'],
  east_asia_to_europe: ['Istanbul', 'Dubai', 'Doha', 'Helsinki'],
  intra_europe: ['Amsterdam', 'Frankfurt', 'Paris', 'London'],
  domestic_us: ['Denver', 'Dallas', 'Chicago', 'Atlanta', 'Charlotte'],
  short_haul: ['Amsterdam', 'Frankfurt', 'Paris', 'London', 'Madrid'],
  long_haul: ['Dubai', 'Istanbul', 'Singapore', 'London', 'Frankfurt'],
};

const ROUTE_LAYOVERS_FAR: Record<RouteType, string[]> = {
  transatlantic_east: ['London', 'Amsterdam', 'Dublin', 'Frankfurt'],
  transatlantic_west: ['New York', 'Boston', 'Washington', 'Chicago'],
  us_to_middle_east: ['Dubai', 'Doha', 'Bahrain'],
  middle_east_to_us: ['New York', 'Washington', 'Chicago'],
  us_to_east_asia: ['Tokyo', 'Seoul', 'Taipei', 'Osaka'],
  east_asia_to_us: ['Honolulu', 'San Francisco', 'Seattle', 'Vancouver'],
  us_to_south_asia: ['Dubai', 'Doha', 'Abu Dhabi'],
  south_asia_to_us: ['New York', 'Washington', 'Chicago'],
  europe_to_east_asia: ['Singapore', 'Bangkok', 'Hong Kong', 'Delhi'],
  east_asia_to_europe: ['Helsinki', 'Frankfurt', 'Amsterdam'],
  intra_europe: ['Zurich', 'Munich', 'Rome', 'Vienna'],
  domestic_us: ['Charlotte', 'Minneapolis', 'Phoenix', 'Salt Lake City'],
  short_haul: ['Rome', 'Barcelona', 'Lisbon', 'Milan'],
  long_haul: ['Singapore', 'Hong Kong', 'Bangkok', 'Dubai'],
};

function pickStopCities(
  rng: () => number,
  stops: number,
  routeType: RouteType,
  ...
  const filtered1 = nearPool.filter(
    (c) => !excludeCities.some((e) => e.toLowerCase() === c.toLowerCase()),
  );
  ...
  const exclude2 = [...excludeCities, city1];
  const filtered2 = farPool.filter(
    (c) => !exclude2.some((e) => e.toLowerCase() === c.toLowerCase()),
  );
...
const ROUTE_PRICE_FACTOR: Record<RouteType, number> = {
  domestic_us: 0.30, intra_europe: 0.25, short_haul: 0.30,
  transatlantic_east: 1.0, transatlantic_west: 1.0,
  us_to_middle_east: 1.20, middle_east_to_us: 1.20,
  us_to_east_asia: 1.30, east_asia_to_us: 1.30,
  us_to_south_asia: 1.35, south_asia_to_us: 1.35,
  europe_to_east_asia: 1.10, east_asia_to_europe: 1.10,
  long_haul: 1.0,
};

const DEFAULT_STOP_DISCOUNT = { nonstop: 1.0, oneStop: 0.82, twoStops: 0.70 };

function getStopPriceFactor(stops: number, discount: typeof DEFAULT_STOP_DISCOUNT): number {
  if (stops === 0) return discount.nonstop;
  if (stops === 1) return discount.oneStop;
  return discount.twoStops;
}

function rollStops(rng: () => number, distanceKm: number): number {
  let direct: number, oneStop: number;
  if (distanceKm < 2000) { direct = 0.95; oneStop = 0.05; }
  else if (distanceKm < 5000) { direct = 0.80; oneStop = 0.18; }
  else if (distanceKm < 8000) { direct = 0.50; oneStop = 0.40; }
  else if (distanceKm < 12000) { direct = 0.35; oneStop = 0.45; }
  else { direct = 0.20; oneStop = 0.45; }
...
interface DepWindow { start: number; end: number }

const ROUTE_DEP_WINDOWS: Record<RouteType, DepWindow[]> = {
  transatlantic_east: [{ start: 17, end: 23 }],
  transatlantic_west: [{ start: 8, end: 16 }],
  us_to_middle_east: [{ start: 20, end: 23 }],
  middle_east_to_us: [{ start: 8, end: 14 }],
  us_to_east_asia: [{ start: 10, end: 14 }, { start: 22, end: 1 }],
  east_asia_to_us: [{ start: 10, end: 18 }],
  us_to_south_asia: [{ start: 19, end: 23 }],
  south_asia_to_us: [{ start: 8, end: 15 }],
  europe_to_east_asia: [{ start: 10, end: 15 }, { start: 20, end: 23 }],
  east_asia_to_europe: [{ start: 10, end: 14 }, { start: 23, end: 3 }],
  intra_europe: [{ start: 6, end: 21 }],
  domestic_us: [{ start: 6, end: 21 }],
  short_haul: [{ start: 6, end: 21 }],
  long_haul: [{ start: 8, end: 22 }],
};

function pickDepartureHour(rng: () => number, routeType: RouteType): { hour: number; minute: number } {
...
function calcBaseDuration(distanceKm: number, rng: () => number): number {
  const base = (distanceKm / 900) * 60 + 30;
  const jitter = (rng() - 0.5) * 30;
  return Math.round(Math.max(45, base + jitter));
}

function getJetStreamMinutes(routeType: RouteType, rng: () => number): number {
  const v = Math.round(rng() * 15);
  switch (routeType) {
    case 'transatlantic_east': return -(25 + v);
    case 'transatlantic_west': return 25 + v;
    case 'us_to_east_asia': return 20 + v;
    case 'east_asia_to_us': return -(20 + v);
    default: return 0;
  }
}

function calcNextDay(depHour: number, depMinute: number, durationMinutes: number): number {
...
const ROUTE_TYPE_CONFIG_KEY: Record<RouteType, string> = {
  domestic_us: 'domestic_us', intra_europe: 'intra_europe', short_haul: 'short_haul',
  transatlantic_east: 'transatlantic', transatlantic_west: 'transatlantic',
  us_to_middle_east: 'us_middle_east', middle_east_to_us: 'us_middle_east',
  us_to_east_asia: 'us_east_asia', east_asia_to_us: 'us_east_asia',
  us_to_south_asia: 'us_south_asia', south_asia_to_us: 'us_south_asia',
  europe_to_east_asia: 'europe_east_asia', east_asia_to_europe: 'europe_east_asia',
  long_haul: 'long_haul',
};

const DEFAULT_CONFIG: FlightGeneratorConfig = {
  resultCount: 8,
  seatsLeftRange: [1, 7],
};

function layoverMinutes(rng: () => number, isDomestic: boolean): number {
...
function generateMultiCityResults(
  params: SearchParams,
  airlines: AirlineInfo[],
  config?: Partial<FlightGeneratorConfig>,
): FlightResult[] {
  if (!params.legs || params.legs.length < 2) return [];
  const cfg = { ...DEFAULT_CONFIG, ...config };

  const legResults = params.legs.map((leg) =>
    generateFlightResults(
      { ...params, from: leg.from, to: leg.to, depart: leg.depart, return: undefined, type: 'oneway' },
      airlines,
      config,
    ),
  );
...
export function generateFlightResults(
  ...
  const fromCoords = getCityCoords(params.from);
  const toCoords = getCityCoords(params.to);
  ...
  const fromCode = getPrimaryAirportCode(params.from);
  const toCode = getPrimaryAirportCode(params.to);
  const fromCity = getCityName(params.from) || params.from;
  const toCity = getCityName(params.to) || params.to;
  ...
  const routeFactor = cfg.routePriceFactor?.[routeConfigKey]
    ?? ROUTE_PRICE_FACTOR[outRouteType]
    ?? 1.0;

  const stopDiscount = cfg.stopDiscount ?? DEFAULT_STOP_DISCOUNT;
  const codeshareProbability = cfg.codeshareProbability ?? 0.15;

  const cabinSeats = cfg.seatsPerCabin?.[params.cabin]
    ?? CABIN_SEATS_RANGE[params.cabin]
    ?? cfg.seatsLeftRange;

  const baggage: BaggageInfo = CABIN_BAGGAGE[params.cabin] ?? CABIN_BAGGAGE.business;
  ...
  for (let i = 0; i < cfg.resultCount; i++) {
    ...
    const returnAirline = rng() > 0.5
      ? shuffledAirlines[Math.floor(rng() * shuffledAirlines.length)]
      : airline;
    ...
    const outStopCity = pickStopCities(rng, stops, outRouteType, [fromCity, toCity]);
    const retStopCity = pickStopCities(rng, returnStops, retRouteType, [toCity, fromCity]);
